...
router.get('/check/:drawId/:betCombination/:betType', requireAuth, async (req, res) => {
  try {
    ...
    // Get bet limit for this bet type
    ...
    if (!betLimit) {
      return res.status(404).json({
        success: false,
        message: 'Bet limit not configured'
      });
    }

    const currentAmount = currentTotal?.totalAmount || 0;
    const isSoldOut = currentAmount >= betLimit.limitAmount;
    ...
    res.json({
      ...
      data: {
        ...
        limitAmount: betLimit.limitAmount,
        remainingAmount: Math.max(0, betLimit.limitAmount - currentAmount),
        isSoldOut: isSoldOut
      }
    });
