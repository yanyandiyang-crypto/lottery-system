...
router.get('/per-draw', requireAuth, async (req, res) => {
  try {
    ...
    const drawSales = {};
    sales.forEach(sale => {
      const drawId = sale.draw.id;
      if (!drawSales[drawId]) {
        drawSales[drawId] = {
          id: drawId,
          drawTime: sale.draw.drawTime,
          drawDate: sale.draw.drawDate,
          status: sale.draw.status,
          winningNumber: sale.draw.winningNumber,
          grossSales: 0,
          ticketCount: 0,
          totalWinnings: 0,
          netSales: 0
      ...
      drawSales[drawId].grossSales += sale.totalAmount;
      drawSales[drawId].ticketCount += sale.ticketCount;
    });

    // Calculate winnings for each draw
    for (const drawId in drawSales) {
      const draw = drawSales[drawId];
      
      // Build winnings query for this specific draw
      let winningsWhereClause = {
      ...
      if (whereClause.userId) {
        ...
        if (whereClause.user.regionId) {
          ...
          winningsWhereClause.ticket.userId = {
            in: areaAgents.map(agent => agent.id)
          };
      ...
      draw.totalWinnings = winningsData._sum.prizeAmount || 0;
      draw.netSales = draw.grossSales - draw.totalWinnings;
    }

    res.json({
      success: true,
      data: {
        draws: Object.values(drawSales)
      }
    });
