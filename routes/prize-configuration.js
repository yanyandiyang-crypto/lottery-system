...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      const hasTable = Array.isArray(existsRows) && existsRows[0] && existsRows[0].count > 0;
      if (!hasTable) {
        // Return sensible defaults when table is missing, so UI can function
        const defaults = [
          {
            betType: 'standard',
            multiplier: 450.0,
          ...
          },
          {
            ...
            baseAmount: 10.0,
            basePrize: 750.0,
            description: 'Rambolito: base ₱10 bet ≈ ₱750 prize (75x)',
            isActive: true,
            createdBy: null,
            updatedBy: null
          }
      ...
      console.log('PrizeConfiguration existence check failed:', e.message);
    }
...
router.post('/', requireAuth, requireSuperAdmin, [
  ...
  try {
    ...
    // If the table doesn't exist, short-circuit and return the payload as if saved
    let hasTable = true;
    try {
      const existsRows = await prisma.$queryRawUnsafe(
        "SELECT COUNT(*)::int AS count FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'prize_configurations'"
      );
      hasTable = Array.isArray(existsRows) && existsRows[0] && existsRows[0].count > 0;
    } catch (e) {
      hasTable = false;
    }
    ...
    if (!hasTable) {
      return res.json({
        success: true,
        message: 'Prize configuration accepted (table not present, using runtime defaults)',
        data: {
          betType,
          multiplier,
          baseAmount,
    ...
    const existingConfig = await prisma.prizeConfiguration.findUnique({ where: { betType } });
