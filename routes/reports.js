...
// @desc    Get sales reports with hierarchical filtering
// @access  Private
router.get('/sales', async (req, res) => {
  try {
    ...
    // Role-based filtering (same as sales.js)
    ...
    } else if (whereClause.user) {
      // Handle role-based filtering for winning tickets
      if (whereClause.user.regionId) {
        const areaAgents = await prisma.user.findMany({
          where: { regionId: whereClause.user.regionId, role: 'agent' },
          select: { id: true }
        });
        winningsWhereClause.userId = {
          in: areaAgents.map(agent => agent.id)
        };
      } else if (whereClause.user.coordinatorId) {
        const coordAgents = await prisma.user.findMany({
          where: { coordinatorId: whereClause.user.coordinatorId, role: 'agent' },
          select: { id: true }
        });
        winningsWhereClause.userId = {
    ...
    const netSales = grossSales - (totalWinnings._sum.prizeAmount || 0);
    ...
    sales.forEach(sale => {
      ...
      salesByDate[date].ticketCount += sale.ticketCount;
      salesByDate[date].agents.add(sale.userId);
    });
    ...
    res.json({
      ...
      data: {
        summary,
        details,
        sales: reportType === 'detailed' ? sales : []
      }
    });
...
router.get('/sales', async (req, res) => {
  try {
    ...
    const totalWinnings = tickets.reduce((sum, ticket) => {
      return sum + ticket.winningTickets.reduce((winSum, wt) => winSum + (wt.prizeAmount || 0), 0);
    }, 0);

    const summary = {
      totalSales: tickets.reduce((sum, ticket) => sum + ticket.totalAmount, 0),
    ...
    res.json({
      ...
      data: {
        ...
        details,
        topPerformers,
        chartData: generateChartData(tickets)
      }
    });
