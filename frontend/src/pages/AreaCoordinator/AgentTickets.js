  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-purple-100">
      <div className="w-full px-4 sm:px-6 lg:px-8 py-4">
        ...
        <ModernCard variant="glass" className="mb-4">
          <div className="p-4 sm:p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center space-x-2">
                  <FunnelIcon className="h-5 w-5 text-purple-600 flex-shrink-0" />
                  <h3 className="text-[clamp(13px,2.6vw,16px)] sm:text-[clamp(14px,2.4vw,18px)] font-semibold text-gray-900">Search & Filters</h3>
                </div>
                <button
                  onClick={() => setShowFilters(v => !v)}
                  className="inline-flex items-center px-3 py-1.5 text-xs sm:text-sm font-medium text-purple-700 bg-purple-100 hover:bg-purple-200 rounded-lg transition-colors"
                  aria-expanded={showFilters}
                  aria-controls="agent-filters-panel"
                >
            ...
            {showFilters && (
            <div id="agent-filters-panel">
              {/* Search Bar */}
              <div className="mb-3 sm:mb-4">
                <div className="relative">
                  <MagnifyingGlassIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-purple-600" />
                  <input
                    type="text"
                    placeholder="Search by ticket number, agent name, or username..."
                    value={searchTerm}
                    onChange={(e) => handleSearch(e.target.value)}
                    className="w-full pl-10 pr-4 py-2 sm:py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white/50 backdrop-blur-sm transition-all duration-200"
                  />
                </div>
              </div>
              ...
              <div className="sm:col-span-2">
                ...
                <div className="flex items-center space-x-2">
                  <div className="relative flex-1">
                    ...
                    className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white/50 backdrop-blur-sm transition-all duration-200 text-xs sm:text-sm"
                    />
                  </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 bg-white/50 backdrop-blur-sm transition-all duration-200 text-xs sm:text-sm"
                >
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-2">Draw Time</label>
                <select
              ...
              <ModernButton
                ...
                onClick={() => refetch()}
                icon={ArrowPathIcon}
                className="w-full text-[clamp(11px,2.6vw,13px)] px-3 py-1.5"
              >
        ...
        <ModernCard variant="elevated" className="animate-fade-in">
          <div className="p-4 sm:p-5">
            <div className="flex items-center space-x-2 mb-6">
              ...
              <h2 className="font-semibold bg-gradient-to-r from-purple-600 to-purple-700 bg-clip-text text-transparent text-[clamp(14px,2.8vw,18px)]">
                Lottery Tickets
              </h2>
              ...
              {filteredTickets.map((t) => (
                <div key={t.id} className="p-3 rounded-xl border border-gray-200 bg-white">
                  <div className="flex items-center justify-between">
                    <div className="font-mono font-semibold text-gray-900">{t.ticketNumber}</div>
                    <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700">{new Date(t.createdAt).toLocaleDateString()}</span>
                  </div>
                  <div className="mt-2 text-sm text-gray-600">
                    {t.user?.fullName || t.user?.username || 'Unknown'} • {formatDrawTimeForTicket(t.draw?.drawTime)}
                  </div>
                  <div className="mt-3 flex items-center justify-between">
                    <div className="text-sm">
                      <div className="text-gray-500">Total</div>
                      <div className="font-semibold text-purple-700">{formatCurrency(t.totalAmount)}</div>
                    </div>
                    <div className="flex gap-2">
                      {canReprint() ? (
                        <ModernButton variant="ghost" size="sm" onClick={() => handleReprint(t)} icon={PrinterIcon} title="Reprint Ticket" className="text-purple-700 hover:text-purple-900">Reprint</ModernButton>
                      ) : (
                        <ModernButton variant="ghost" size="sm" onClick={() => handleViewDetails(t)} icon={EyeIcon} className="text-purple-700 hover:text-purple-900">View</ModernButton>
                      )}
                    </div>
                  </div>
                </div>
