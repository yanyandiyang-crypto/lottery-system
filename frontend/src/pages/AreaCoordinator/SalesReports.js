      ...
      <div className="w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        ...
        >
          <ModernButton
            ...
            variant="success"
            size="md"
            className="text-[clamp(11px,2.6vw,13px)] px-3 py-1.5"
          >
        ...
        <ModernCard className="mb-6">
          <div className="bg-gradient-to-r from-purple-50 to-purple-100 px-4 py-3 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 text-[clamp(16px,3.2vw,20px)]">Report Filters</h3>
                  <p className="text-xs sm:text-sm text-gray-600 mt-0.5">Configure your report</p>
                </div>
              </div>
              <button
          ...
          {showFilters && (
          <div id="report-filters-panel" className="p-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 text-xs sm:text-sm"
                />
              </div>
              <div>
          ...
          {activeTab === 'overview' && reportData && (
            <div className="space-y-8">
              ...
              {reportData.details && reportData.details.length > 0 && (
                <div className="bg-white shadow overflow-hidden sm:rounded-md hidden md:block">
            ...
            {activeTab === 'userSales' && reportData && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3 md:hidden">
                  {reportData.sales && reportData.sales.slice(0, 20).map((sale, idx) => (
                    <div key={idx} className="p-3 rounded-xl border border-gray-200 bg-white">
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-sm font-semibold text-gray-900">{sale.user?.fullName || 'Unknown'}</div>
                          <div className="text-xs text-gray-500">{sale.draw?.drawTime ? formatDrawTime(sale.draw.drawTime) : 'N/A'} • {new Date(sale.createdAt).toLocaleDateString()}</div>
                        </div>
                        <div className="text-sm font-bold text-purple-700">{formatCurrency(sale.totalAmount)}</div>
                      </div>
                    </div>
                  ))}
                  {reportData.details && reportData.details.map((day, idx) => (
                    <div key={`d-${idx}`} className="p-3 rounded-xl border border-gray-200 bg-white">
                      <div className="flex items-center justify-between">
                        <div className="text-sm font-semibold text-gray-900">{new Date(day.date).toLocaleDateString()}</div>
                        <div className="text-sm font-bold text-purple-700">{formatCurrency(day.totalAmount)}</div>
                      </div>
                      <div className="mt-2 text-xs text-gray-600">Tickets: {day.ticketCount.toLocaleString()} • Active Agents: {day.agents}</div>
                    </div>
                  ))}
                </div>
              </div>
