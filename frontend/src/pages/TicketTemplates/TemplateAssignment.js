...
const TemplateAssignment = () => {
  ...
  const assignDesign = async (ev) => {
    ...
    try {
      setLoading(true);
      // Map options to backend design ids (reuse 3 for centered variant to satisfy 1-5 validation)
      const templateDesign = selectedTemplateKey === 'umatik' ? 3
        : selectedTemplateKey === 'umatik-center' ? 3
        : 1;
  ...
  const previewHtml = useMemo(() => {
    ...
    if (selectedTemplateKey === 'umatik-center') {
      // Reuse design 3 but different templateType so renderer can pick centered variant
            ...
            <div className="mt-1 flex gap-3">
              ...
              <label className="inline-flex items-center gap-2 text-sm">
                <input
                  ...
                  className="text-blue-600"
                  value="umatik-center"
                  checked={selectedTemplateKey === 'umatik-center'}
                  onChange={() => setSelectedTemplateKey('umatik-center')}
                  disabled={loading}
                />
                Umatik (Centered Logo)
              </label>
            </div>
          </div>
