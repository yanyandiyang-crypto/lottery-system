...
const SalesPerDraw = () => {
  ...
  const fetchDailyStats = async () => {
    try {
      ...
      // Fetch today's stats
      const todayResponse = await salesAPI.getPerDrawSales({ ...params, date: today });
      const todayData = todayResponse.data?.data?.draws || [];
      const todaySales = todayData.reduce((sum, draw) => sum + draw.grossSales, 0);
      const todayTickets = todayData.reduce((sum, draw) => sum + draw.ticketCount, 0);

      // Fetch this week's stats (simplified - you might want to implement a proper week endpoint)
      const weekResponse = await salesAPI.getPerDrawSales({ ...params, date: startOfWeek.toISOString().split('T')[0] });
      const weekData = weekResponse.data?.data?.draws || [];
      const weekSales = weekData.reduce((sum, draw) => sum + draw.grossSales, 0);
      const weekTickets = weekData.reduce((sum, draw) => sum + draw.ticketCount, 0);

      // Fetch this month's stats (simplified - you might want to implement a proper month endpoint)
      const monthResponse = await salesAPI.getPerDrawSales({ ...params, date: startOfMonth.toISOString().split('T')[0] });
      const monthData = monthResponse.data?.data?.draws || [];
      const monthSales = monthData.reduce((sum, draw) => sum + draw.grossSales, 0);
      const monthTickets = monthData.reduce((sum, draw) => sum + draw.ticketCount, 0);
