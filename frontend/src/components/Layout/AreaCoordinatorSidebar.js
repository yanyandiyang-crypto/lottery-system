...
// Area Coordinator has regional management access
...
const AreaCoordinatorSidebar = ({ isCollapsed = false, onToggleCollapse }) => {
  const { user } = useAuth();
    ...
    <div className={`bg-white shadow-lg border-r border-gray-200 transition-all duration-300 ${
      ...
      <div className="flex-shrink-0 p-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          {!isCollapsed && (
            <div>
              <h1 className="text-xl font-bold text-gray-800">Area Coordinator</h1>
              <p className="text-sm text-gray-600">Regional Management</p>
            </div>
          )}
          <button
      ...
      {/* User Info */}
      {!isCollapsed && user && (
        <div className="flex-shrink-0 p-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-purple-100">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-r from-purple-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold">
              {user.username?.charAt(0).toUpperCase() || 'AC'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">
                {user.username || 'Area Coordinator'}
              </p>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-6 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100" style={{
        ...
        {Object.entries(areaCoordinatorNavigation).map(([groupKey, items]) => {
...
export default AreaCoordinatorSidebar;
