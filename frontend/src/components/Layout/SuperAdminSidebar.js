...
const superAdminNavigation = {
  ...
  management: [
    ...
    { name: 'Admin Management', href: '/admin-management', emoji: '🏢' },
  ...
  system: [
    { name: 'Prize Configuration', href: '/prize-configuration', emoji: '💵' },
    { name: 'Template Assignment', href: '/template-assignment', emoji: '🎨' },
    { name: 'Security Audit', href: '/admin/audit', emoji: '🛡️' },
...
const SuperAdminSidebar = ({ isCollapsed = false, onToggleCollapse }) => {
  const { user } = useAuth();
  ...
  return (
    <div className={`bg-white shadow-lg border-r border-gray-200 transition-all duration-300 ${
      ...
      {/* User Info */}
      {!isCollapsed && user && (
        <div className="flex-shrink-0 p-4 border-b border-gray-200 bg-gradient-to-r from-red-50 to-red-100">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 bg-gradient-to-r from-red-500 to-red-600 rounded-full flex items-center justify-center text-white font-bold">
              {user.username?.charAt(0).toUpperCase() || 'SA'}
            </div>
