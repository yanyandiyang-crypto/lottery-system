...
public class POSDeviceManager {
    ...
    /**
     * DEBUG: Get detailed device information
     */
    ...
    /**
     * Print bitmap image to thermal printer using ESC/POS commands
     */
    public boolean printBitmap(android.graphics.Bitmap bitmap) {
        ...
        try {
            // Resize bitmap to fit thermal printer width (384 pixels for 58mm, 576 for 80mm)
            int maxWidth = 384; // Default to 58mm printer
            ...
            command[5] = (byte) ((widthBytes >> 8) & 0xFF);
            
            // Height in dots
            command[6] = (byte) (height & 0xFF);
            command[7] = (byte) ((height >> 8) & 0xFF);
            
            outputStream.write(command);
            
            // Convert bitmap to monochrome and send
            for (int y = 0; y < height; y++) {
                byte[] line = new byte[widthBytes];
            ...
            Log.e(TAG, "Failed to print bitmap: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
