-- Step 2: Add claiming fields and use the new 'claimed' enum value
...
-- Update existing winning tickets to use 'validated' status
UPDATE tickets 
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_log') THEN
        ...
        VALUES (
            1, -- System user
            'MIGRATION_EXECUTED',
            '{"migration": "005b_add_claiming_fields", "description": "Added claiming fields and indexes", "timestamp": "' || NOW() || '"}',
            NOW()
        );
    END IF;
END$$;
