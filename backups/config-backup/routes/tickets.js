  ...
  try {
    ...
    if (idempotencyKey) {
      ...
      if (Array.isArray(exists) && exists[0]?.ticket_id) {
        ...
        if (existingTicket) {
          return res.json({ success: true, message: 'Idempotent replay', ticket: existingTicket, remainingBalance: null });
        }
      }
    }
    ...
    for (const bet of bets) {
      ...
      if (!/^\d{3}$/.test(bet.betCombination)) {
        return res.status(400).json({ success: false, message: 'Bet combination must be 3 digits' });
      }
      if (!(bet.betAmount > 0)) {
    ...
    // Pre-validate limits atomically (all-or-nothing)
    // Build cumulative map per (betType, number)
    const reqTotals = new Map();
    for (const bet of bets) {
      const key = `${bet.betType}::${bet.betCombination}`;
      reqTotals.set(key, (reqTotals.get(key) || 0) + Number(bet.betAmount));
    }

    // Load limits for bet types involved
    const betTypes = [...new Set(bets.map(b => b.betType))];
    const limits = await prisma.betLimit.findMany({ where: { betType: { in: betTypes }, isActive: true } });
    const limitMap = new Map(limits.map(l => [l.betType, Number(l.limitAmount)]));
    for (const bt of betTypes) {
      if (!limitMap.has(bt)) {
        return res.status(400).json({ success: false, message: `Bet limit not configured for ${bt}` });
      }
    }

    // Load current totals for all (number,type) in this request
    const whereOr = Array.from(reqTotals.keys()).map(k => {
      const [bt, num] = k.split('::');
      return { drawId: drawIdNum, betCombination: num, betType: bt };
    });
    const currentTotals = whereOr.length
      ? await prisma.currentBetTotal.findMany({ where: { OR: whereOr } })
      : [];
    const currentMap = new Map(currentTotals.map(t => [`${t.betType}::${t.betCombination}`, Number(t.totalAmount)]));

    // Validate each unique (type,number) cumulative total <= limit
    for (const [key, addAmt] of reqTotals.entries()) {
      const [bt, num] = key.split('::');
      const existing = currentMap.get(key) || 0;
      const limit = limitMap.get(bt) || 0;
      if (existing + addAmt > limit) {
        return res.status(400).json({ success: false, message: `Bet limit exceeded for ${num} (${bt}). Current: ₱${existing.toFixed(2)}, Request: ₱${addAmt.toFixed(2)}, Limit: ₱${limit.toFixed(2)}` });
      }
    }
    ...
    // Use atomic transaction service for balance + then create bets & totals within prisma transaction
    ...
    if (result.success) {
      // Create bets and update totals within a single transaction for consistency
      await prisma.$transaction(async (tx) => {
        ...
        // Update totals
        for (const bet of bets) {
          await tx.currentBetTotal.upsert({
            where: {
            ...
            update: {
              totalAmount: { increment: bet.betAmount },
...
router.get('/', async (req, res) => {
  try {
    ...
    // Search filter (ticket number or bet combination)
    if (search) {
      where.OR = [
